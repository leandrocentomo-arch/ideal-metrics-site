...
MOTOR = r"""  /* ---------------- CONHECIMENTO: as manchas por tras dos aros ---------------- */
  (function(){
    ...
    if(f && fi) ditherVivo({raiz:f, planos:[{el:f, lum:fi.getAttribute('data-lum')}], classeCanvas:'af-gl', revelar:'visivel',
      cores:[[21.2,50.9,80.6],[24.2,56.2,88.1],[103.0,146.1,189.4],[250,249,245]], pincel:.7, trilhaSoNaFigura:true, biasReacao:.10});
  })();
...
def secao(L):
    return '\n'.join([
        ...
        arcos.lista(),
        '  </div>',
        '  <div class="ar-foto" aria-hidden="true"><img src="img/conhecimento-foto-bayer.webp?v=7" '
        'data-lum="img/conhecimento-foto-lum.webp?v=7" alt="" width="480" height="867" loading="lazy" decoding="async"></div>',
        '  </div>',
        '</section>',
    ])
